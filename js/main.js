...
let problemData = {};
        ...
        html += `
            ...
            <div id="callbackForm_${problemKey}" class="callback-form" style="margin-top: 20px;">
                ...
                <form onsubmit="return submitCallbackFormInline(event, '${problemKey}')">
                    ...
                    <!-- Dringlichkeit: Nur am Samstag beide Optionen zeigen -->
                    <div id="urgencySection_${problemKey}" style="margin-top: 30px; padding: 20px; background: #fff3cd; border: 3px solid #ffc107; border-radius: 12px; box-shadow: 0 4px 12px rgba(255, 193, 7, 0.2); -webkit-box-sizing: border-box; box-sizing: border-box;">
                        ...
                        <div style="margin: 0;">
                            ...
                            <div style="display: -webkit-box; display: -ms-flexbox; display: flex; -webkit-box-orient: vertical; -webkit-box-direction: normal; -ms-flex-direction: column; flex-direction: column; gap: 12px; margin-top: 10px;">
                                <label class="saturday-radio-label">
                                    <input type="radio" name="urgency_${problemKey}" value="monday" checked>
                                    <span class="saturday-radio-text">${t.formUrgencyMonday}</span>
                                </label>
                                <label class="saturday-radio-label" id="saturdayOption_${problemKey}">
    ...
    function showCallbackFormInline(detailsElementId, problemKey) {
        ...
        if (form) {
            ...
            // Prüfe, ob heute Samstag ist (6 = Samstag)
            const today = new Date();
            const isSaturday = today.getDay() === 6;
            
            // Samstag-Option und gelbe Box nur am Samstag anzeigen
            const saturdayOption = document.getElementById(`saturdayOption_${problemKey}`);
            const urgencySection = document.getElementById(`urgencySection_${problemKey}`);
            
            if (isSaturday) {
                // Samstag: Beide Optionen zeigen, gelbe Box anzeigen
                if (saturdayOption) saturdayOption.style.display = 'block';
                if (urgencySection) urgencySection.style.display = 'block';
            } else {
                // Kein Samstag: Samstag-Option verstecken, gelbe Box verstecken
                if (saturdayOption) saturdayOption.style.display = 'none';
                if (urgencySection) urgencySection.style.display = 'none';
                // Automatisch "monday" vorauswählen
                const mondayRadio = document.querySelector(`input[name="urgency_${problemKey}"][value="monday"]`);
